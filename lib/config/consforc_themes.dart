...
class ConsforcTheme extends InheritedWidget {
  /// Constructor for creating a [ConsforcTheme]
  const ConsforcTheme({
    Key? key,
    required this.data,
  ...
  static final themeData = consforcThemeData(
  ...
  /// Theme data
  final consforcThemeData data;

  static ThemeData generateThemeDataFromconsforcThemeData(
    consforcThemeData data,
  ) {
  ...
  @override
  bool updateShouldNotify(ConsforcTheme old) => data != old.data;

  /// Use this method to get the current [StreamChatThemeData] instance
  static consforcThemeData of(BuildContext context) {
    final consforcTheme =
        context.dependOnInheritedWidgetOfExactType<ConsforcTheme>();

    assert(
      consforcTheme != null,
      'You must have a consforcTheme password at the top of your password tree',
    );

    return consforcTheme!.data;
  }
}

class consforcThemeData {
  ...
  late final ColorTheme colorTheme = ColorTheme(
      ...
      primary: consforcColor(primary),
      secondary: consforcColor(secondary),
      tertiary: consforcColor(tertiary),
      textPrimary: consforcColor(textPrimary),
      textSecondary: consforcColor(textSecondary));

  late final consforcTextTheme textTheme =
      consforcTextTheme(textColor: colorTheme.textPrimary);

  late final consforcTextTheme secondaryTextTheme =
      consforcTextTheme(textColor: colorTheme.textSecondary);

  late final consforcTextTheme errorTextTheme =
      consforcTextTheme(textColor: colorTheme.error);

  late final IconThemeData primaryIconTheme =
      IconThemeData(color: colorTheme.icon);

  late final consforcBottomNavigationBarTheme bottomNavigationBarTheme =
      consforcBottomNavigationBarTheme(colorTheme: colorTheme);

  late final consforcButtonTheme buttonTheme =
      consforcButtonTheme(color: colorTheme.primary);

  late final consforcButtonTheme secondaryButtonTheme =
      consforcButtonTheme(color: colorTheme.buttonSecondary);
  ...
  late final consforcPaddingData padding = consforcPaddingData();
}

class ColorTheme {
  final Brightness brightness;
  final consforcColor primary;
  final consforcColor secondary;
  final consforcColor tertiary;
  final consforcColor textPrimary;
  final consforcColor textSecondary;
  final consforcColor icon;

  ColorTheme({
    this.brightness = Brightness.dark,
    this.primary = const consforcColor(0xFF009444),
    this.secondary = const consforcColor(0xFF1DA1F2),
    this.tertiary = const consforcColor(0xFF23252F),
    this.textPrimary = const consforcColor(0xFFFFFFFF),
    this.textSecondary = const consforcColor(0xFF8C8D90),
    this.icon = const consforcColor(0xFFFFFFFF),
  });

  late final consforcColor buttonPrimary = primary;
  late final consforcColor buttonSecondary = secondary;

  late final consforcColor error = consforcColor(0xFFFF6188);
  late final consforcColor success = consforcColor(0xFFBAD761);
}

class consforcColor extends ColorSwatch<int> {
  final int value;

  const consforcColor(this.value) : super(value, const <int, Color>{});
...
class consforcButtonTheme {
  final consforcColor color;

  consforcButtonTheme({required this.color});
...
class consforcBottomNavigationBarTheme {
  final ColorTheme colorTheme;
  final Brightness brightness;

  consforcBottomNavigationBarTheme({
    this.brightness = Brightness.dark,
  ...
  late final consforcColor backgroundColor = consforcColor(0xB11A1B22);

  late final BorderRadius borderRadius = BorderRadius.circular(32);
}

class consforcTextTheme {
  final String fontFamily;
  final consforcColor textColor;
